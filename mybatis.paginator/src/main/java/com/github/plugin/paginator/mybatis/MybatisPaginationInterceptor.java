package com.github.plugin.paginator.mybatis;
...
import com.github.plugin.paginator.mybatis.jdbc.dialet.Dialect;
...
public class MybatisPaginationInterceptor implements Interceptor {
	...
	private Integer queryCount(Invocation inv, final Object[] queryArgs, MappedStatement ms, BoundSql boundSql, String sql)
		...
		// 设置总数
		Integer total = (Integer) ((List<Object>) result).get(0) == null ? 0 : (Integer) ((List<Object>) result).get(0);
		return total;
	}
	...
	/**
	 * 该方法查询记录数数据不正确 第一查询的数据正确，当数据库的数据增加时，在此查询的结果可能是上一次查询的结果。 如果将connection
	 * 设置为自动提交，则结果数据正确.
	 * <p>
	 * 查询总记录数
	 * </p>
	 ...
	 * @throws SQLException
	 */
	public int queryTotal(String sql, MappedStatement mappedStatement, BoundSql boundSql) throws SQLException {
	...
	private MappedStatement copyFromMappedStatementToCountMappedStatement(MappedStatement ms, BoundSql boundSql, String sql) {
		BoundSql newBoundSql = copyFromBoundSql(ms, boundSql, sql);
		return copyFromMappedStatementToCountMappedStatement(ms, new BoundSqlSqlSource(newBoundSql));
	}
	...
	public static final String SUFFIX = "_PuginCount";
	private static final List<ResultMapping> EMPTY_RESULTMAPPING = new ArrayList<ResultMapping>(0);

	private MappedStatement copyFromMappedStatementToCountMappedStatement(MappedStatement ms, SqlSource newSqlSource) {
		String id = ms.getId() + SUFFIX;
		Builder builder = new Builder(ms.getConfiguration(), id, newSqlSource, ms.getSqlCommandType());
