...
public class HttpClient4Util {
     ...
     * @return
     */
    private static HttpClient createExtraHttpClient(boolean https, Integer port) {
        HttpParams httpParams = setConfig();
        try {
            KeyStore trustStore = KeyStore.getInstance(KeyStore.getDefaultType());
            trustStore.load(null, null);
            HttpClient4SSLSocketFactory sf = new HttpClient4SSLSocketFactory(trustStore);
            sf.setHostnameVerifier(HttpClient4SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);

            SchemeRegistry registry = new SchemeRegistry();
            registry.register(new Scheme("http", PlainSocketFactory.getSocketFactory(), port == null ? 80 : port.intValue()));
            if (https) {
                registry.register(new Scheme("https", sf, port == null ? 443 : port.intValue()));
            }
//                ClientConnectionManager ccm = new ThreadSafeClientConnManager(params, registry);
            PoolingClientConnectionManager conMgr = new PoolingClientConnectionManager(registry);
